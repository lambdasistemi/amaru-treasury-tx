...
- **FR-005**: `tx-build --report` MUST emit a JSON build-output envelope with required top-level `intent` and `result` fields. The top-level `intent` MUST be the full originating unified-intent JSON value with the same shape as the standalone intent JSON and MUST be passed through unchanged after successful intent decoding.
- **FR-006**: The `result` field MUST be a union: either a structured failure value explaining why no transaction was created, or a success object with required `tx-cbor` and `report` fields. Success `tx-cbor` MUST be the exact unsigned transaction CBOR encoded as lowercase hex; success `report` MUST be the builder's mechanical explanation of the relevant transaction facts from issue #72 so downstream renderers and reviewers do not need to parse CBOR to understand the transaction.
...
- **Build Output Envelope**: The machine-readable artifact written by `tx-build --report`. It has top-level `intent` and `result`. The intent is pass-through context for the attempted build. A success result contains `tx-cbor` and nested mechanical `report`; a failure result contains structured failure data explaining why no transaction was created.
