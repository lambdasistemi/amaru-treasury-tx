#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 185-reorganize-core PR.
#
...
if [ -d specs/185-reorganize-core ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/185-reorganize-core/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
