#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 87-devnet-reorganize-smoke PR.
#
...
if [ -d specs/87-devnet-reorganize-smoke ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/87-devnet-reorganize-smoke/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
