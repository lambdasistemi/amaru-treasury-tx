...
The machine-readable JSON Schema contract is committed at
[`docs/assets/intent-schema.json`](../../../docs/assets/intent-schema.json).
It is generated by
`Amaru.Treasury.IntentJSON.Schema.intentJsonSchema` and printed by
the `amaru-treasury-intent-schema` executable.

Update protocol:

1. Change the Haskell intent records / parser.
2. Update `Amaru.Treasury.IntentJSON.Schema`.
3. Run `just update-schema`.
4. Run `just schema-check` and the unit tests.

CI runs the schema drift check through
`checks.x86_64-linux.schema`. The unit suite validates the
checked-in swap build fixture, the swap-wizard golden intent, and
fresh JSON emitted by `wizardToTreasuryIntent`; it also asserts
that an `action` / payload-block mismatch is rejected by the
schema.

## 8. Breaking changes vs feature 002 / feature 004 contracts
