...
- [x] T045 [US4] Confirm `nix develop --quiet -c just schema-check` and `nix develop --quiet -c just unit IntentJSONSchema` both pass.
...
- [x] T049 [US5] Run `nix develop --quiet -c just smoke` and confirm withdraw pipe/help smoke passes.
...
- [x] T055 Run full local gate: `nix develop --quiet -c just ci && nix develop --quiet -c just cabal-check`.
