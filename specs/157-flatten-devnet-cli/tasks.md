...
- [ ] T060 RED: add `test/unit/Amaru/Treasury/Build/NetworkGuardSpec.hs` with seven cases — for each of the seven init sub-action intents, build a fixture with `network: mainnet` and assert that `runFromIntentEither` returns a typed `BuildError` *before* any N2C connection (use a dummy `ChainContext` that fails loudly if touched). ALSO: extend the schema's `network` enum to include `"devnet"` (flagged at slice-2 review — schema currently lists only mainnet/preprod/preview). (`test/unit/Amaru/Treasury/Build/NetworkGuardSpec.hs`, `test/fixtures/intent/…-mainnet.json` as needed, `lib/Amaru/Treasury/IntentJSON/Schema.hs`, `docs/assets/intent-schema.json`)
