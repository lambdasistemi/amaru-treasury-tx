#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 187-reorganize-wizard-runner PR.
#
...
if [ -d specs/187-reorganize-wizard-runner ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/187-reorganize-wizard-runner/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
