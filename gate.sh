#!/usr/bin/env bash
# Mechanical gate for the #379 PR (pending SPA deep-link fallback).
# Present for the PR's life; dropped in the final `chore: drop gate.sh`
...
# lack of that tooling and are unrelated to #379's SPA fallback. They
# remain covered by CI's hermetic unit check. Skip exactly those
# describe-paths in the local per-commit gate; everything else must pass.
...
# Ticket-specific focused proof added by the implementation slice:
#   nix develop --quiet -c just unit "SpaFallback"
