#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 184-exclude-utxo PR.
#
...
if [ -d specs/184-exclude-utxo ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/184-exclude-utxo/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
