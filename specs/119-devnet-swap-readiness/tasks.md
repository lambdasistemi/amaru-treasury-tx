...
- [ ] T006 [US1] RED: add a focused `swap-ready readiness` Hspec example to `test/devnet/Amaru/Treasury/Devnet/SmokeSpec.hs` that requires readiness summary/registry fields.
- [ ] T007 [US1] RED: run `nix develop --quiet -c cabal test devnet-tests -O0 --test-show-details=direct --test-option=--match --test-option='swap-ready readiness'` and record the expected failure in this file.
...
- [ ] T009 [US1] Add a pure test in `test/unit/Amaru/Treasury/ConstantsSpec.hs` or a new focused module that requires the public SundaeSwap V3 `order.spend` artifact and expected script hash.
- [ ] T010 [US1] Add the public order-validator artifact under `assets/plutus/` or an equivalent reviewed source path, with provenance documented in `assets/plutus/README.md`.
- [ ] T011 [US1] Add Haskell constants/helpers for the order-validator source, blob, and hash in the existing constants/registry modules without changing swap builder semantics.
- [ ] T012 [US1] GREEN: run the focused artifact/hash test and record the passing command output.
...
- [ ] T013 [US1] Add `swap-ready` phase parsing to `scripts/smoke/devnet-local` and the `all` phase only if the review accepts including readiness in `all`.
- [ ] T014 [US1] Add a `swap-ready` selector and smoke action in `test/devnet/Amaru/Treasury/Devnet/SmokeSpec.hs`.
- [ ] T015 [US1] Reuse the DevNet reference-script publication helpers to publish the order validator reference script and wait for the reference UTxO.
- [ ] T016 [US2] Write `swap-ready/registry.json`, `swap-ready/summary.json`, and `swap-ready/provenance.json` with the fields in `contracts/devnet-swap-readiness.md`.
- [ ] T017 [US2] Assert the readiness registry contains enough order-build inputs for #84: order address, order script hash, order script ref, network magic, source commit, and artifact paths.
- [ ] T018 [US2] GREEN: run `nix develop --quiet -c just devnet-smoke swap-ready` and record the run directory, script hash, reference UTxO, and registry path.
...
- [ ] T024 [US3] Update `docs/local-devnet-smoke.md` with `swap-ready` usage, artifacts, diagnostics, and boundary text.
- [ ] T025 [US3] Update `README.md`, `docs/release.md`, and `CHANGELOG.md` if verified readiness evidence changes release-facing behavior or release notes.
- [ ] T026 [US3] Update #132 issue metadata with verified run evidence and add a #84 handoff comment naming the readiness artifact.
- [ ] T027 [US3] Run `./llm/reviews/local-119-devnet-swap-readiness/gate.sh` and record the result.
- [ ] T028 [US3] Commit the accepted vertical slice as `test(devnet): register sundaeswap readiness` or another reviewed conventional title.
...
- RED/GREEN: pending; first implementation task is T006/T007.
