#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 186-reorganize-wizard-parser PR.
#
...
if [ -d specs/186-reorganize-wizard-parser ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/186-reorganize-wizard-parser/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
