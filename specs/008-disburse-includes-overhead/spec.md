...
- The permissions multisig (`Disburse` arm of `TreasurySpendRedeemer` in [`pragma-org/amaru-treasury` `validators/permissions.ak`](https://github.com/pragma-org/amaru-treasury/blob/main/validators/permissions.ak)) authorizes a disbursement based on signer approval only and does **not** inspect `amount` magnitude. **Verified** (FR-007 satisfied, T001 of `tasks.md`): at sha `15817e6bcd6da7121f93022508572784af94a270` (origin/main, 2026-05-08), the `Disburse { .. }` branch of the `withdraw` validator at `validators/permissions.ak:59` matches the variant with `{ .. }` — no field of `Disburse` is extracted — and dispatches to `approved_by_owner_and_someone_else`, a pure multisig check (`scope.ak`); no comparison against `amount` is performed in `permissions.ak`. The on-chain `amount`-magnitude constraint that does exist lives in Sundae's [`treasury-contracts/validators/treasury.ak`](https://github.com/SundaeSwap-finance/treasury-contracts/blob/main/validators/treasury.ak) `Disburse { amount } -> disburse.logic` path (the `equal_plus_min_ada` check), and the previous assumption shows that constraint only loosens as `amount` grows.
