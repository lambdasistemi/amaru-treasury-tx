...
# lack of that tooling and are entirely unrelated to #365 (introspect /
# verify-witness do no RDF). They remain covered by CI's hermetic unit
# check. Skip exactly those describe-paths in the local per-commit gate;
# everything else (including the new Introspect/VerifyWitness specs)
# must pass.
