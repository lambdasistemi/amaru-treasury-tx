#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 218-reorganize-wizard-mainnet PR.
#
...
if [ -d specs/218-reorganize-wizard-mainnet ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/218-reorganize-wizard-mainnet/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
