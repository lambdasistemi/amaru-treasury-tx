#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 217-reorganize-wizard-utxo-selection PR.
#
...
if [ -d specs/217-reorganize-wizard-utxo-selection ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/217-reorganize-wizard-utxo-selection/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
