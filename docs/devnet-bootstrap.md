...
This page documents the DevNet-only bootstrap flow: how the
registry-init, stake-reward-init, governance-withdrawal-init, and
disburse sub-transactions are produced through the shipped
`wizard → tx-build → witness → submit` chain. It is operator
reference material for driving a fresh DevNet, and the live-DevNet
evidence captured along the way. For mainnet operation see the
[Quickstart](quickstart.md) and the per-action recipe pages.

## Bootstrap via `tx-build --intent`
