...
- **FR-010a**: The dashboard MUST render **every field** of the `InspectReport` JSON document returned by `GET /v1/treasury-inspect`. No field may be silently dropped. Specifically: chain tip (slot + block hash when present), deployment anchor (scope-owners outref), and for each scope — treasury address, treasury script hash, every UTxO (outref, lovelace, USDM, every other asset, deployed-at, datum when present), totals, and every pending SundaeSwap order with its datum details.
- **FR-010b**: Where a field is a raw identifier (txid, outref, address, script hash, policy ID), the rendering MUST be **resolved as well as possible**: txids and outrefs are rendered as outbound cardanoscan links; addresses are displayed in a short form with the full bech32 available on hover or click; script hashes and policy IDs are displayed short with the full hex available on hover; datums are rendered as a readable tree, not raw CBOR.
