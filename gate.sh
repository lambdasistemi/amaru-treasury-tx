#!/usr/bin/env bash
# gate.sh — local pre-push gate for the 230-reorganize-tx-build-batching PR.
#
...
if [ -d specs/230-reorganize-tx-build-batching ]; then
  open=$(grep -nE '^\s*-\s*\[ \]\s*T[0-9]+' specs/230-reorganize-tx-build-batching/tasks.md 2>/dev/null || true)
  if [ -n "$open" ]; then
