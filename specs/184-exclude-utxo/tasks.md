...
- [ ] T006 [US3] RED: assert that running `swap-wizard --exclude-utxo X --extra-tx-in X` (same outref both flags) exits non-zero with a structured `Contradiction` error before any chain query. Test fails because the flag is unwired.
- [ ] T007 [P] [US1] RED: assert that with two candidate wallet UTxOs (`A` 92.56 ADA, `B` 19.76 ADA) and `--exclude-utxo <A>`, the wizard's emitted intent has `wallet.txIn = B` and the log records `swap-wizard: excluded utxo <A> (operator-supplied)`.
