...
- [X] T001 Read issue #79, PR #77, and current `lib/Amaru/Treasury/Build.hs` failure paths.
- [X] T002 Run baseline focused tests: `nix develop --quiet -c just unit "Build"` and record output in the PR body.

## Phase 2: Foundational

- [X] T003 [P] Add RED unit tests in `test/unit/Amaru/Treasury/BuildSpec.hs` for stable codes/messages for each normalized build diagnostic.
- [X] T004 [P] Add RED report-envelope test in `test/unit/Amaru/Treasury/ReportSpec.hs` or `BuildSpec.hs` proving `BuildFailure` receives normalized code/message.
- [X] T005 [P] Add RED unit tests in `test/unit/Amaru/Treasury/BuildSpec.hs` proving structured context can be added to `BuildException` with `mapException` for pure exception expressions and with the project helper for `IO` exceptions.
- [X] T006 Add `BuildError`, `BuildDiagnostic`, `BuildErrorContext`, code renderer, and message renderer in `lib/Amaru/Treasury/Build.hs` or a new `lib/Amaru/Treasury/Build/Error.hs`.
- [X] T007 Add conversion from upstream `BuildError ()` and local runner failures into `BuildError`.
- [X] T008 Add `withBuildErrorContext`, `mapBuildExceptionContext`, and an IO wrapper/catcher helper for structured context enrichment.
...
- [X] T009 [US1] Add RED runner or CLI test for a swap build failure in `test/unit/Amaru/Treasury/BuildSpec.hs`.
- [X] T010 [US1] Add `runFromIntentEither :: ChainContext -> SomeTreasuryIntent -> IO (Either BuildError BuildResult)` in `lib/Amaru/Treasury/Build.hs`.
...
- [X] T012 [US1] Keep `runFromIntent` as a compatibility wrapper that throws a typed `BuildException` with normalized `displayException`.
- [X] T013 [US1] Use `mapException` where pure exception mapping applies, and the IO wrapper/catcher helper where a `throwIO` boundary adds action/phase/context to `BuildException`.
...
- [X] T015 [US2] Update `writeFailureReport` call sites in `app/amaru-treasury-tx/Main.hs` to use `BuildError` code/message for expected build failures.
