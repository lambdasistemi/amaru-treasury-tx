...
It is a Turtle overlay generated by `book-export` from the vendored
canonical journal metadata (`journal/2026/metadata.json` — provenance in
[`journal/README.md`](journal/README.md)). For every treasury scope it
names the owner key, treasury address, and treasury / permissions /
registry script hashes with human labels, so a resolver can turn raw
Cardano identifiers into named scopes and roles.

Import it into the
[Cardano Swiss Knife Library][csk-library] via the **Book URL** input to
resolve Amaru treasury transactions live from the published location — no
operator-local file to hand around. The book mirrors the upstream journal
faithfully, including the known `contingency` `registry_script.hash` typo
(see [`journal/README.md`](journal/README.md)); it is a resolution aid,
not an authority on on-chain script hashes.

CI drift-checks the book (the `book` flake check fails closed if the
committed asset diverges from regeneration) and the docs workflow deploys
it to GitHub Pages. See the [Treasury book page][book] for details.
...
[book]: https://lambdasistemi.github.io/amaru-treasury-tx/book/
[csk-library]: https://lambdasistemi.github.io/cardano-swiss-knife/library/
