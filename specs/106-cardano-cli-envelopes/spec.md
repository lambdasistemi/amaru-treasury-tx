...
An operator just produced an unsigned tx via `tx-build` and wants to hand it to a signer who only accepts the `cardano-cli` `Unwitnessed Tx ConwayEra` envelope shape on disk.
...
**Independent Test**: `echo deadbeef | amaru-treasury-tx envelope-tx` emits JSON whose `type` is `"Unwitnessed Tx ConwayEra"` and whose `cborHex` field is `"deadbeef"`.

**Acceptance Scenarios**:

1. **Given** raw CBOR hex on stdin, **When** running `envelope-tx`, **Then** stdout contains a single JSON envelope with `type: "Unwitnessed Tx ConwayEra"`, the canonical `description`, and `cborHex` equal to the stdin bytes verbatim.
...
After `attach-witness` merges all witnesses, the operator wants the signed result as a `Signed Tx ConwayEra` envelope — so they can submit through `cardano-cli conway transaction submit --tx-file` or hand the file to a tool that expects the `cardano-cli` shape.
...
1. **Given** raw signed CBOR hex on stdin, **When** running `envelope-signed-tx`, **Then** stdout is a JSON envelope with `type: "Signed Tx ConwayEra"`, the canonical `description`, and `cborHex` equal to the stdin bytes.
...
2. **Given** an `Unwitnessed Tx ConwayEra` envelope on stdin, **When** running `de-envelope`, **Then** the same extraction works — `de-envelope` does not care which Conway envelope kind it is given.
...
The operator pipes a stale `Unwitnessed Tx BabbageEra` envelope into the pipeline. We catch it before it reaches the node.
...
- **Trailing newline on input hex** — `envelope-*` commands strip a single trailing newline from stdin (the common pipe artefact); they do **not** strip internal whitespace. The body of the `cborHex` field is the trimmed value.
...
- **FR-1** New subcommand `envelope-tx`: reads raw CBOR hex on stdin, writes a single JSON envelope on stdout with `type: "Unwitnessed Tx ConwayEra"`, the canonical `description`, and `cborHex` equal to the (trailing-newline-trimmed) stdin bytes.
- **FR-2** New subcommand `envelope-witness`: same shape, `type: "TxWitness ConwayEra"`.
- **FR-3** New subcommand `envelope-signed-tx`: same shape, `type: "Signed Tx ConwayEra"`.
...
- **SC-5** A stale `Unwitnessed Tx BabbageEra` envelope (checked-in fixture) piped through `de-envelope` exits non-zero with a stderr line containing the literal string `BabbageEra`.
