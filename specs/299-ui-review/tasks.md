...
- [X] T299-S1 — `frontend/src/App.purs:389` — replace the `⎘ Copy inspect JSON` text node with `<md-icon>content_copy</md-icon>` + `<span>Copy inspect JSON</span>`.
- [X] T299-S1 — `frontend/src/App.purs:735` (`copyRow`) — render the `icon` value inside an `<md-icon>` element; flip the label vocabulary to `"content_copy"` / `"check"` instead of `"⎘"` / `"✓"`.
- [X] T299-S1 — `frontend/src/Shell.purs:109-114` — topbar theme button: drop the literal text label; add `<md-icon>` (`dark_mode` when `themeLabel == "Dark"`, else `light_mode`); add `title="Switch to <themeLabel> theme"`; add `<span class="visually-hidden">{themeLabel}</span>` child.
- [X] T299-S1 — `frontend/dist/styles.css` — add `.visually-hidden` utility class (position: absolute; 1×1px; clip).
- [X] T299-S1 — `frontend/dist/styles.css` — add `.topbar__theme-btn` styling (44×44, `inline-grid`, hover / outline tokens).
- [X] T299-S1 — `frontend/dist/styles.css` — add `.v-copy--block md-icon { font-size: 18px; }` rule.
- [X] T299-S1 — `nix build --quiet .#frontend` green at HEAD.
- [X] T299-S1 — Playwright after-capture (orchestrator-run) at `/`, `/operate`, `/books` × {1280, 390, 320} dropped under `frontend/test/ui-review/299/after/` (slice A subset).
- [X] T299-S1 — Commit `feat(299): md-icon migration + a11y theme toggle` with `Tasks: T299-S1` trailer.
